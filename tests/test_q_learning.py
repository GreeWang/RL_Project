...
from q_learning import QLearningAgent, SarsaAgent
...
def test_sarsa_update_matches_on_policy_formula() -> None:
    rng = np.random.default_rng(0)
    agent = SarsaAgent(
        n_states=4,
        n_actions=2,
...
def test_sarsa_update_disables_bootstrap_when_terminated() -> None:
    rng = np.random.default_rng(0)
    agent = SarsaAgent(
        n_states=4,
        n_actions=2,
